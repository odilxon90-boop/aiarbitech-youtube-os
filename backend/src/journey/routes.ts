...
export function registerJourneyRoutes(app: FastifyInstance): void {
  app.get('/api/v1/journey/health', async (request) => successResponse({ status: 'ALIVE' }, request.correlationId));
  app.get('/api/v1/journey/dashboard/summary', async (request) => {
  ...
  app.get<{ Querystring: { dateRange?: 'week' | 'month'; range?: 'week' | 'month'; channel?: string } }>('/api/v1/journey/analytics/overview', async (request) => {
  ...
  app.post<{ Body: { message: string } }>('/api/v1/journey/ai/chat/send', async (request) => {
