...
describe('Monitoring health checks', () => {
  it('reports service, database, gateway, and request health', async () => { const app = await createApp(); const health = await app.inject({ method: 'GET', url: '/health' }); const db = await app.inject({ method: 'GET', url: '/health/db' }); const gateway = await app.inject({ method: 'GET', url: '/health/gateway' }); expect(health.json().data).toMatchObject({ status: 'ok' }); expect(db.json().data.connectivity).toBe('MOCK_CONNECTED'); expect(gateway.json().data.networkRequestPerformed).toBe(false); });
