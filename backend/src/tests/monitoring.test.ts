...
describe('Monitoring health checks', () => {
  it('reports root and health service status', async () => { const app = await createApp(); const root = await app.inject({ method: 'GET', url: '/' }); const health = await app.inject({ method: 'GET', url: '/health' }); const db = await app.inject({ method: 'GET', url: '/health/db' }); const gateway = await app.inject({ method: 'GET', url: '/health/gateway' }); expect(root.json().data).toMatchObject({ status: 'ok' }); expect(health.json().data).toMatchObject({ status: 'ok' }); expect(db.json().data.connectivity).toBe('MOCK_CONNECTED'); expect(gateway.json().data.networkRequestPerformed).toBe(false); });
