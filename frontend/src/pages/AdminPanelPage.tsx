...
export function AdminPanelPage({ initialData }: { initialData?: AdminSummary }) {
  if (!initialData) {
    return <section className="admin-panel" aria-label="Admin Panel"><h2>Admin Panel</h2><p>Loading admin panel…</p></section>;
  }
  return <section className="admin-panel" aria-labelledby="admin-panel-title"><div className="admin-panel__header"><div><p className="eyebrow">Administration</p><h2 id="admin-panel-title">Admin Panel</h2></div><span className="foundation-badge">admin:access</span></div><div className="admin-grid"><UserList users={initialData.users} /><ChannelModeration channels={initialData.channels} /><AIConfigForm initialConfig={initialData.aiConfig} /><HealthCard health={initialData.health} /><AuditLogList entries={initialData.auditLogs} /></div></section>;
}
