...
const videos = [
  ['AI tools that changed my workflow', 'Published', '124K', '8.4%'],
  ['Build a channel with automation', 'Processing', '—', '—'],
  ['The creator economy in 2026', 'Published', '86K', '7.1%'],
];
...
export function CreatorDashboard() {
  ...
  const createDraft = () => {
    ...
    setNotice('AI Director queued your video brief.');
    setDirectorPrompt('');
  };
    ...
    <div className="min-h-screen bg-[#070b16] text-white">
      ...
      <main className="mx-auto max-w-[1440px] px-6 py-8 lg:px-10">
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[['Total views', '2.84M', '+18.4%', 'text-cyan-300'], ['Subscribers', '184.2K', '+7.8%', 'text-violet-300'], ['Creator revenue', '$12,480', '+22.1%', 'text-emerald-300'], ['Engagement rate', '8.62%', '+1.4%', 'text-amber-300']].map(([label, value, change, color]) => <div key={label} className="rounded-2xl border border-white/10 bg-white/[.04] p-5"><p className="text-xs text-slate-500">{label}</p><p className={`mt-3 text-3xl font-black ${color}`}>{value}</p><p className="mt-2 text-xs text-emerald-300">{change} <span className="text-slate-500">vs last month</span></p></div>)}
        </section>
