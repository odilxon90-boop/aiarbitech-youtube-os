import type { ContractCompatibility, Gate0BGovernance, GlobalEcosystemConnectionStatus, GovernanceArtifact, HealthStatus, PlatformFoundationStatus, PlatformManifest } from './types';
...
export class HttpPlatformFoundationClient implements PlatformFoundationClient{
 ...
 async loadFoundationStatus(signal?:AbortSignal):Promise<PlatformFoundationStatus>{
  ...
  const [manifest,live,ready,connection,...governanceValues]=await Promise.all([this.get<PlatformManifest>('/platform/manifest',signal),this.get<LiveResponse>('/health/live',signal),this.get<ReadyResponse>('/health/ready',signal),this.get<GlobalEcosystemConnectionStatus>('/platform/compatibility',signal),...Object.values(paths).map((path)=>this.get<GovernanceArtifact>(path,signal))]);
  ...
  const health:HealthStatus={live:live.status,ready:ready.status,environment:ready.checks.environment};return{manifest,health,connection,governance:{...governance,contractCompatibility:governance.contractCompatibility as ContractCompatibility}};
 }
}
