import type { CapabilityRegistry, ContractCompatibility, Gate0BGovernance, GlobalEcosystemConnectionStatus, GovernanceArtifact, HealthStatus, PlatformFoundationStatus, PlatformHealthManifest, PlatformManifest, RegistrationSummary } from './types';
...
export class HttpPlatformFoundationClient implements PlatformFoundationClient{
 ...
 async loadFoundationStatus(signal?:AbortSignal):Promise<PlatformFoundationStatus>{
  ...
  const keys=Object.keys(paths) as (keyof Gate0BGovernance)[];const governance=Object.fromEntries(keys.map((key,index)=>[key,governanceValues[index]])) as unknown as Gate0BGovernance;governance.capabilities=governance.capabilities as CapabilityRegistry;governance.healthManifest=governance.healthManifest as PlatformHealthManifest;
